  ...
  async createTriggers(conn) {
    ...
    await this.executeQuery(`
      ...
      BEGIN
        ...
        DECLARE item_tax DECIMAL(10,2);
        DECLARE item_discount DECIMAL(10,2);
        DECLARE gst_rate DECIMAL(5,2);
        
        -- Calculate discount amount
        IF NEW.discount_type = 'percentage' THEN
          SET item_discount = (NEW.price * NEW.quantity) * (NEW.discount / 100);
        ELSE
          SET item_discount = NEW.discount;
        END IF;
        
        -- Calculate subtotal after discount
        SET item_subtotal = (NEW.price * NEW.quantity) - item_discount;
        
        -- Calculate tax if GST slab is applied
        IF NEW.gst_slab_id IS NOT NULL THEN
          SELECT percentage INTO gst_rate FROM gst_slabs WHERE id = NEW.gst_slab_id;
          SET item_tax = item_subtotal * (gst_rate / 100);
          
          -- Update the item with calculated tax
          UPDATE receipt_items 
          SET tax_amount = item_tax 
          WHERE id = NEW.id;
        ELSE
          SET item_tax = 0;
        END IF;
        ...
        SET 
          ...
          total = total + item_subtotal + item_tax
    ...
    await this.executeQuery(`
      ...
      FOR EACH ROW
      BEGIN
        DECLARE old_subtotal DECIMAL(10,2);
        DECLARE new_subtotal DECIMAL(10,2);
        DECLARE old_tax DECIMAL(10,2);
        DECLARE new_tax DECIMAL(10,2);
        DECLARE old_discount DECIMAL(10,2);
        DECLARE new_discount DECIMAL(10,2);
        DECLARE gst_rate DECIMAL(5,2);
        
        -- Calculate old values
        IF OLD.discount_type = 'percentage' THEN
          SET old_discount = (OLD.price * OLD.quantity) * (OLD.discount / 100);
        ELSE
          SET old_discount = OLD.discount;
        END IF;
        SET old_subtotal = (OLD.price * OLD.quantity) - old_discount;
        SET old_tax = OLD.tax_amount;
        
        -- Calculate new values
        IF NEW.discount_type = 'percentage' THEN
          SET new_discount = (NEW.price * NEW.quantity) * (NEW.discount / 100);
        ELSE
          SET new_discount = NEW.discount;
        END IF;
        SET new_subtotal = (NEW.price * NEW.quantity) - new_discount;
        
        -- Calculate tax if GST slab is applied
        IF NEW.gst_slab_id IS NOT NULL THEN
          SELECT percentage INTO gst_rate FROM gst_slabs WHERE id = NEW.gst_slab_id;
          SET new_tax = new_subtotal * (gst_rate / 100);
          
          -- Update the item with calculated tax if it changed
          IF new_tax != NEW.tax_amount THEN
            UPDATE receipt_items 
        ...
        -- Update receipt totals with the difference
        UPDATE receipts
        SET 
          subtotal = subtotal - old_subtotal + new_subtotal,
          total_tax = total_tax - old_tax + new_tax,
          total = total - (old_subtotal + old_tax) + (new_subtotal + new_tax)
        WHERE id = NEW.receipt_id;
      END
    `, [], conn);
    ...
    await this.executeQuery(`
      ...
      BEGIN
        ...
        -- Calculate discount amount
        IF OLD.discount_type = 'percentage' THEN
          SET item_discount = (OLD.price * OLD.quantity) * (OLD.discount / 100);
        ELSE
          SET item_discount = OLD.discount;
        END IF;
        
        -- Calculate subtotal after discount
        SET item_subtotal = (OLD.price * OLD.quantity) - item_discount;
        
        -- Update receipt totals by subtracting the deleted item
        UPDATE receipts
        SET 
          subtotal = subtotal - item_subtotal,
          total_tax = total_tax - OLD.tax_amount,
          total = total - (item_subtotal + OLD.tax_amount)
