...
class DatabaseSetup {
  ...
  async createDatabase() {
    ...
    await this.executeQuery(`CREATE DATABASE IF NOT EXISTS ${dbConfig.database}`);
      ...
      BEGIN
        ...
        ELSE
          ...
          SET total_due_balance = (
            SELECT SUM(amount_due) FROM due_records WHERE user_id = NEW.user_id AND is_paid = FALSE
          )
    ...
    await this.executeQuery(`
      ...
      BEGIN
        ...
        SET is_complete = (
          ...
          NEW.store_name IS NOT NULL AND 
          NEW.store_address IS NOT NULL AND 
          NEW.store_contact IS NOT NULL AND
          NEW.store_contact REGEXP '^[0-9]{10}$'
        );
  ...
  async createProcedures() {
    ...
    await this.executeQuery(`
      CREATE PROCEDURE IF NOT EXISTS check_profile_complete(IN user_id INT, OUT is_complete BOOLEAN)
      BEGIN
        SELECT 
          (name IS NOT NULL AND 
           store_name IS NOT NULL AND 
           store_address IS NOT NULL AND 
           store_contact IS NOT NULL AND
           store_contact REGEXP '^[0-9]{10}$')
        INTO is_complete
        FROM users
        WHERE id = user_id;
      END;
