...
// Export application pool using CommonJS
  ...
  async connect() {
    ...
    try {
      this.pool = mysql.createPool({
        ...dbConfig,
        database: undefined, // Connect without specifying database initially
      });
      ...
      BEGIN
        ...
        ELSE
          ...
          SET total_due_balance = (
            SELECT COALESCE(SUM(amount_due), 0) FROM due_records WHERE user_id = NEW.user_id AND is_paid = FALSE
          )
