...
class DatabaseSetup {
  ...
  private async useDatabase(): Promise<void> {
    try {
      ...
      console.error(`Error switching to database '${dbConfig.database}':`, error);
      throw error;
    }
  }
  ...
  private async createTriggers(): Promise<void> {
    try {
      ...
      await this.executeQuery(`
        DROP TRIGGER IF EXISTS after_due_record_change;
      `);
      await this.executeQuery(`
      ...
      await this.executeQuery(`
        DROP TRIGGER IF EXISTS after_due_record_update;
      `);
      await this.executeQuery(`
