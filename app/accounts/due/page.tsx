...
export default function DuePage() {
  ...
  const handleMarkAsPaid = async (record: DueRecord) => {
    ...
    try {
      ...
      toast.success(`Payment from ${record.customerName} recorded successfully`)
  ...
  const getTotalDueAmount = () => {
    return dueRecords.reduce((total, record) => total + (record.amountDue || 0), 0)
  }
      ...
      {dueRecords.length > 0 ? (
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {dueRecords.map((record) => (
                      <tr key={record.id}>
                        ...
                        <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500">
                          <span className={isPastDue(record.expectedPaymentDate) ? "text-red-600 font-medium" : ""}>
                        ...
                        <td className="px-4 py-4 whitespace-nowrap text-center">
                          <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                            ...
                            {isPastDue(record.expectedPaymentDate) ? "Overdue" : "Pending"}
                          </span>
                        </td>
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <span className={`text-xs ${
                          isPastDue(record.expectedPaymentDate) ? "text-red-600 font-medium" : "text-gray-700"
                        }`}>
                          {formatDate(record.expectedPaymentDate)}
                        </span>
                      </div>
        ...
        <Card className="max-w-2xl mx-auto">
          <CardContent className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No Due Records</h3>
            <p className="text-gray-500 mb-4">
              Great! All payments have been received. There are no outstanding dues at the moment.
            </p>
            <Link href="/accounts">
